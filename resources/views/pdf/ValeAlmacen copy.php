...
$conceptosTodos = array_values($element->Conceptos->all());
$hConceptos = 60;
$incrementoBase = $hConceptos / 11;
$pdf->SetFont('Arial','',$hConceptos/8);
$conceptosPorVale = dividirConceptosPorVale($pdf, $conceptosTodos, $incrementoBase, anchoDescripcion(25, 210));
$totalVales = max(1, count($conceptosPorVale));

for ($paginaVale = 1; $paginaVale <= $totalVales; $paginaVale++) {
    if (($paginaVale - 1) % 2 === 0) {
        $pdf->AddPage('P','A4');
    }
    ...
    $index = 0;
    $py+=$incremento;
    $limiteConceptosY = $py + ($incremento * 9);
    while($py < $limiteConceptosY){
        $datos1 = $conceptos[$index] ?? (object) ['Descripcion' => '', 'Cantidad' => ''];
        $altoRenglon = altoRenglonConcepto($pdf, $datos1->Descripcion ?? '', $incremento, anchoDescripcion($px, $tw));

        if ($py + $altoRenglon > $limiteConceptosY && $index >= count($conceptos)) {
            break;
        }
    ...
    $pdf->SetFont('Arial','',$h/8);
    $pdf->Cell($w,6,"",'LRB',1,"C",false);
    $pdf->SetXY(170, $offsetY + 99);
    $pdf->SetFont('Arial','B',6);
    $pdf->Cell(30,4,"Pagina ".$paginaVale." de ".$totalVales,0,1,"R",false);
}
    // fin y entrega del pdf 
$pdf->Output();
exit;

function printrow($t1,$t2,$t3,$t4,$t5,$t6,$pdf,$px,$py,$tw,$incremento,$lineHeight = null){
        $lineHeight = $lineHeight ?? $incremento;
        $x1=$px;
        $pdf->SetXY($x1,$py);
        $w1=28;
        ...
        $w1=$tw-$x1-54-10;
        $pdf->SetXY($x1,$py);
        $pdf->Rect($x1,$py,$w1,$incremento);
        $pdf->MultiCell($w1,$lineHeight,utf8_decode($t3),0,"C",false);
        $x1+=$w1;
        $w1=18;
        $pdf->SetXY($x1,$py);
...
function anchoDescripcion($px,$tw){
        $x1 = $px + 28 + 9;
        return $tw - $x1 - 54 - 10;
}
function altoRenglonConcepto($pdf, $text, $incremento, $width) {
        $lineas = getLineCount($pdf, $text, $width);
        return $incremento * $lineas;
}
function dividirConceptosPorVale($pdf, $conceptos, $incremento, $width) {
        ...
        foreach ($conceptos as $concepto) {
                $alto = altoRenglonConcepto($pdf, $concepto->Descripcion ?? '', $incremento, $width);

                if ($altoActual > 0 && $altoActual + $alto > $limite) {
                        $chunks[] = $chunk;
                        $chunk = [];
                        $altoActual = 0;
                }
