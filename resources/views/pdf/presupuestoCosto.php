...
foreach ($presupuesto->conceptos as $concepto){
    ...
    if($grupoant != $grupo){  
      ...
      $total = $total  + ($concepto->Cantidad*$concepto->Costo); 
    ...
    //utf8_decode es para que escriba bien
    //los acentos. 
    $pdf->Row(array($concepto->DatosConcepto->num,$concepto->Cantidad,$concepto->DatosConcepto->descripcion,amoneda($concepto->Costo),amoneda($concepto->Cantidad*$concepto->Costo)));
//un arreglo con alineacion de cada celda
//OTro arreglo pero con el contenido
//utf8_decode es para que escriba bien
//los acentos. 
