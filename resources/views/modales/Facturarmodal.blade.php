    ...
    <div class="modal-dialog zdmw-60pct modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <button data-id="4" class="btn btn-info">K</button>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary prefacturas" id="timbrarprefacturas" hidden>Timbrar</button>
...
$('#timbrarfacturas').on('click', function (){
    ...
    $.ajax({
            ...
            data:{
                ids: id_presupuestos.split(",").map(Number),
                contrato:contrato,
            },
            success: function(response) {
                ...
                $.each(listaconceptos, function(index, element) {
                    ...
                    data[index].economico=element.economico;
                    data[index].placas=element.placas;
                    data[index].kilometraje=element.KmDeIngreso;
                    data[index].NSolicitud=element.NSolicitud;
                    data[index].descripcionMO=element.descripcionMO;
                    data[index].importep=element.importe;
                    data[index].contrato=response.contrato;
                });
                Swal.fire({
                title: '¿Estás seguro?',
                text: "",
                ...
                cancelButtonText: 'No, Cancelar'
                    ...
                    if (result.isConfirmed) {
                    $.ajax({
                    url: "{{route('2025.cfe.facturar.varias')}}",
                    type: "post",
                    data:{
                        "_token": "{{ csrf_token() }}",
                        data:data,
...
$('#timbrarprefacturas').on('click', function (){
    let factura={};
    let data=[];
    let emisor_id= $('#emisorrfc').val();
    ...
    $.ajax({
            ...
            success: function(response) {
                ...
                $.each(listaconceptos, function(index, element) {
                    data[index] = {};
                    data[index].id=element.presupuesto_id;
                    data[index].economico=element.economico;
                    data[index].placas=element.placas;
                    data[index].kilometraje=element.kilometraje;
                    data[index].NSolicitud=element.NSolicitud;
                    data[index].descripcionMO=element.descripcionMO;
                    data[index].importep=element.importe;
                    data[index].contrato=response.contrato;
...
$('#guardarfacturas').on('click', function (){
    const modulop1 = @json($modulo);
    const aniop1 = @json($anio);
    const contratop1 = @json($contrato);
    let factura={};
    let data=[];
    const id_presupuestos=$(this).attr('data-id');
    console.log(id_presupuestos);
    factura.empresa_id=$('#empresasfactura').val();
    factura.tipo_impuesto_local=$('#tipo_impuesto_local').val();
    factura.tipo_comprobante=$('#tipo_comprobante').val();
    factura.uso_cfdi=$('#uso_cfdi').val();
    factura.moneda=$('#moneda').val();
    factura.fpago=$('#fpago').val();
    ...
    $.ajax({
            ...
            success: function(response) {
                ...
                cancelButtonText: 'No, Cancelar'
                    ...
                    if (result.isConfirmed) {
                    $.ajax({
                    url: "{{route('2025.cfe.guardar.facturar.varias')}}",
                    type: "post",
                    data:{
                        "_token": "{{ csrf_token() }}",
                        data:data,
